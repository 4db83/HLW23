...
% 
% 	% make Table
% 	tabS2 = table(par_names2, S2_initVals, S2_Rfile, S2_replicated, S2_MLE_g, S2_MLE_M0g);% , S2_replicated, S2_MLE_g);
...
% 	% ---------------------------------------------------------------------------------------------------
% clc; clf;
% % F-stats = tstat^2 CORRECT
% % (eout_S2_M0g.tstat_t(:,end).^2)
% % (eout_S2_M0g.Fstat_ols)
% % plot(eout_S2_M0g.YX.tT)
% 
% % plot(eout_S2_bl.YX.tT)
% % plot(eout_S2_bl.YX.tT(:,5))
% % a = plotacf(eout_S2_bl.YX.tT(:,5)); a(1:3)
% % a = plotacf(eout_S2_bl.YX.tT(:,2)); a(1:3)
% % a = plotacf(eout_S2_bl.YX.tT(:,4)); a(1:3)
...
%   [theta3.gzCS, LL3.gzCS, retCodegz,~, gradOpt3.gzCS, hessOpt3.gzCS] = csminwel('LogLike_Stage3_HLW_SSF_sigma_gz', [theta3.g;	.01], 1*eye(length(theta3.gz)), [], 1e-10, 1e5, data_in,	other_pars_S3);
% 	se_gz_CS = diag(sqrt(inv(hessOpt3.gzCS)));
	...
	% STANDARD ERRORS FOR FULL MODEL IF NEEDED: 
	% no need to minus the Hessian because it is coming from minimization problem
	se_gz = diag(sqrt(inv(hessOpt3.gzO)));
...
% 	print2screen(tabS3([1:10 12:13 11],[1 3:end]), '%16.10f')
	...
	legNames_= {'HLW R-Files';
							...
							'MLE($\sigma_g|\hat{\lambda}_z^{\mathrm{Correct}})$';
							'MLE($\sigma_g,\sigma_z)$'};

%   'MLE($\sigma_g|\lambda_z^{\mathcal{M}_0})$';
	...
	if PLOT_ON
		f1 = figure(1); clf; 
    set(f1,'WindowState','maximized','Position',[1441 641 1200 1843]);
		for jj = 1:numS
		% ---------------------------------------------------------------------------------------------------
			subplot(numS,1,jj); 
...
%     figout_name1 = [ COUNTRY '_estimates_' KFS_name '_' diff_name '_' SMPL_END '_big'];
%  		PRINT_PLOT_TO_PDF == 1; print2pdf(figout_name1, '../../EJ/graphics/');
	end
  ...
  if PRINT_FACTORS_EXCEL == 1
  	...
  	% filtered factors MLE.M0(g)
  	xlswrite(factors_xls_name, cellstr('MLE.M0(g|Lambda_g^M0)'), 	'Filtered', 'G1')
...
%   	xlswrite(factors_xls_name, HLW.g.att									, 			'Filtered', 'L3') 
  	...
  	xlswrite(factors_xls_name, cellstr('MLE.M0(g|Lambda_g^M0)'), 	'Smoothed', 'G1')
...
%     CI = 2
		f2 = figure(2); clf; 
%     set(f2,'WindowState','maximized','Position',[1441 641 1200 1843]);
		set(groot,'defaultLineLineWidth',1.75); 
 		% PLOTTING DIMENSIONS FOR SUB-FIGURES ---------------------------------------------------------------
 		p.fs = 20;			% FONT SIZE                                       
 		p.df = 29;			% DATE FREQUENCY                                  
%     p.df = 34;			% DATE FREQUENCY                                  
 		p.aj = -0.15;	% SUBTITLE POSITION ADJUSTMENT                  
 		p.dm = @(x)([.785-(x-1)*.22 .86 .180]);% FIG DIMENSION      
 		% call as: setplot(p.dm(2),p.fs,1)
 		% ---------------------------------------------------------------------------------------------------
...
% 		setoutsideTicks
% 		add2yaxislabel
		subtitle('(i) Misspecified: $GY_t = a_{y}(L)\tilde{y}_{t}-a_{r}(L)r_{t}-a_{g}g_{t-1}-a_{0}$', p.aj, p.fs+1, 1)
							% subtitle('Demeand series', -1.22, 14, 1)					

%     if ismember(CI,[3])
% 		legendflex({'Time varying \bf\phi';
% 								'Constant \bf\phi'}, ...
% 								'Interpreter','Tex','anchor', 8*ones(1,2))
%     end
...
% 		setoutsideTicks
% 		add2yaxislabel
		addsubtitle('(ii) Correctly specified: $GY_{t}=a_{y}(L)\tilde{y}_{t}-a_{r}(L)[r_{t}-4g_{t}]$', p.aj, p.fs+1, 1)
...
% 		legendflex({'Time varying \bf\phi';
% 								'Constant \bf\phi'}, ...
% 								'Interpreter','Tex','anchor', [1 4])
%     end

% 		fstat_fig_outname = [ COUNTRY '_Fstat_MUE2_' SMPL_END ];
% 		if PRINT_PLOT_TO_PDF == 1; print2pdf(fstat_fig_outname, latex_graphics_dir); end

%     fstat_fig_outname = [ COUNTRY '_Fstat_MUE2_' SMPL_END '_big'];
%  		PRINT_PLOT_TO_PDF == 1; print2pdf(fstat_fig_outname, '../../EJ/graphics/');
