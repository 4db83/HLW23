...
kalman.standard.errors <- function(t.end, states, theta, y.data, x.data, stage,
    ...
    if (date.se < start.phi.se) { params.exclude.list <- c(params.exclude.list, "phi") } # If date is before 2020Q1, exclude phi from SE computation
    if (!is.na(fix.phi)) { params.exclude.list <- c(params.exclude.list, "phi") } # If fix.phi is not NA, exclude phi from SE computation
    if (use.kappa) {
      for (k in 1:(dim(kappa.inputs)[1])) {
        if (kappa.inputs$lower.bound[k]==kappa.inputs$upper.bound[k]) { params.exclude.list <- c(params.exclude.list, kappa.inputs$name[k]) } # If kappa is fixed (upper bound = lower bound), exclude from SE computation
      }
    }
    ...
    t.stats[params.se] <- abs(theta[params.se]) / bse # vector of length theta; NA for params.exclude
